...
class TestAPIKeyConfig:
    ...
    def test_valid_api_key(self):
        ...
        config = APIKeyConfig(
            key="AIzaSyD-1234567890abcdefghijklmnopqrstuv", provider="Google"
        )
    ...
    def test_google_key_format_warning(self, mock_warn):
        ...
        config = APIKeyConfig(
            key="invalid-google-key-format-1234567890", provider="Google"
        )
    ...
    def test_openai_key_format_warning(self, mock_warn):
        ...
        # Valid format
        config = APIKeyConfig(key="sk-" + "a" * 48, provider="OpenAI")
        assert not mock_warn.called

        # Invalid format
        mock_warn.reset_mock()
        config = APIKeyConfig(key="openai-invalid-key", provider="OpenAI")
        assert mock_warn.called
    ...
    def test_unknown_provider_no_validation(self):
        ...
        config = APIKeyConfig(key="any-format-key-123", provider="UnknownProvider")
...
class TestEnvironmentConfig:
    ...
    def test_default_values(self):
        ...
        for key in [
            ...
            "OPENAI_API_KEY",
            "ANTHROPIC_API_KEY",
            "VIRTUAL_AGORA_LOG_LEVEL",
            "VIRTUAL_AGORA_LOG_DIR",
        ]:
