...
class TestModeratorNodeIntegration:
    ...
    def sample_state_with_vote(self):
        ...
        return VirtualAgoraState(
            ...
            proposed_topics=["Climate Change Policy", "Economic Impact", "Technology Solutions"],
            topics_info={
    ...
    @pytest.mark.asyncio
    async def test_story_37_minority_considerations_workflow(self, moderator_agent, sample_state_with_vote):
        ...
        sample_state_with_vote["active_vote"]["minority_voters"] = result1["active_vote"]["minority_voters"]
        
        # Step 2: Collect minority considerations
        with patch.object(moderator_agent, 'collect_minority_consideration', 
            ...
            result2 = await nodes.collect_minority_considerations_node(sample_state_with_vote)
            ...
            mock_collect.assert_called_once_with("agent2", "Climate Change Policy", sample_state_with_vote)
        
        # Update state with considerations
        sample_state_with_vote["active_vote"]["minority_considerations"] = result2["active_vote"]["minority_considerations"]
        
        # Step 3: Incorporate minority views into summary
        with patch.object(moderator_agent, 'incorporate_minority_views', 
            ...
            result3 = await nodes.incorporate_minority_views_node(sample_state_with_vote)
            
            assert "topic_summaries" in result3
            assert "stronger enforcement mechanisms" in result3["topic_summaries"]["Climate Change Policy"]
            mock_incorporate.assert_called_once()

    @pytest.mark.asyncio
    async def test_story_38_report_writer_workflow(self, moderator_agent, sample_state_with_vote):
        ...
        with patch.object(moderator_agent, 'define_report_structure', 
                         new_callable=AsyncMock) as mock_structure:
            ...
            result1 = await nodes.initialize_report_structure_node(sample_state_with_vote)
        ...
        with patch.object(moderator_agent, 'generate_report_section', 
                         new_callable=AsyncMock) as mock_section:
            ...
            assert "synthesizes insights" in result2["report_sections"]["Executive Summary"]
    ...
    @pytest.mark.asyncio
    async def test_story_39_agenda_modification_workflow(self, moderator_agent, sample_state_with_vote):
        ...
        with patch.object(moderator_agent, 'request_agenda_modification', 
            ...
            result1 = await nodes.request_agenda_modifications_node(sample_state_with_vote)
            
            assert "pending_agenda_modifications" in result1
            assert len(result1["pending_agenda_modifications"]) == 3
            assert "Carbon Pricing Mechanisms" in result1["pending_agenda_modifications"][0]
            assert mock_request.call_count == 3  # Called for each agent
        
        # Update state with modifications
        sample_state_with_vote["pending_agenda_modifications"] = result1["pending_agenda_modifications"]
        
        # Step 2: Synthesize agenda changes
        with patch.object(moderator_agent, 'synthesize_agenda_modifications', 
                         new_callable=AsyncMock) as mock_synthesize:
        ...
        with patch.object(moderator_agent, 'collect_agenda_vote', 
                         new_callable=AsyncMock) as mock_vote:
            ...
            with patch.object(moderator_agent, 'synthesize_agenda_votes', 
                             new_callable=AsyncMock) as mock_synthesize_votes:
                ...
                result3 = await nodes.facilitate_agenda_revote_node(sample_state_with_vote)
    ...
    @pytest.mark.asyncio
    async def test_end_to_end_topic_conclusion_workflow(self, moderator_agent, sample_state_with_vote):
        ...
        with patch.object(moderator_agent, 'collect_minority_consideration', 
                         new_callable=AsyncMock) as mock_collect, \
             patch.object(moderator_agent, 'incorporate_minority_views', 
                         new_callable=AsyncMock) as mock_incorporate:
            
            mock_collect.return_value = "Strong enforcement needed for effective climate policy"
            mock_incorporate.return_value = (
                "Updated summary with minority view: Strong enforcement needed for effective climate policy"
            )
            ...
            assert len(sample_state_with_vote["active_vote"]["minority_considerations"]) == 1
            assert "Strong enforcement needed" in result3["topic_summaries"]["Climate Change Policy"]
