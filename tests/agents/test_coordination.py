...
class TestVoteCollector:
    ...
    def test_get_conclusion_vote_tally(self):
        ...
        yes_count, no_count, minority_voters = (
            self.vote_collector.get_conclusion_vote_tally()
        )
...
class TestAgentCoordinator:
    ...
    def test_coordinate_discussion_round(self):
        ...
        for i, agent in enumerate(self.mock_agents.values()):
            agent.generate_discussion_response.return_value = (
    ...
    def test_coordinate_discussion_round_with_muted_agent(self):
        ...
        self.mock_agents["agent1"].generate_discussion_response.return_value = (
            "Response 1"
        )
        self.mock_agents["agent3"].generate_discussion_response.return_value = (
            "Response 3"
        )
...
class TestAgentCoordinatorErrorHandling:
    ...
    def test_handle_agent_errors_in_proposals(self, mock_executor_class):
        ...
        error_future.result.return_value = (
            "agent2",
            [],
    ...
    def test_handle_timeout_in_discussion(self):
        ...
        self.mock_agents["agent2"].generate_discussion_response.side_effect = Exception(
            "Timeout"
        )
    ...
    def test_error_statistics_tracking(self):
        ...
        self.mock_agents["agent1"].generate_discussion_response.side_effect = Exception(
            "Test error"
        )
...
class TestCoordinationIntegration:
    ...
    def test_full_coordination_workflow(self):
        ...
        for i in range(3):
            ...
            agent.generate_discussion_response.return_value = (
                f"Discussion from {agent_id}"
            )
            agent.vote_on_topic_conclusion.return_value = (
                True,
                f"Conclusion from {agent_id}",
            )
        ...
        with patch("concurrent.futures.ThreadPoolExecutor") as mock_executor_class:
            ...
            for agent_id in mock_agents.keys():
                ...
                future.result.return_value = (
                    agent_id,
                    (True, f"Conclusion from {agent_id}"),
                )
    ...
    def test_thread_safety(self):
        ...
        for agent_id in agent_ids:
            assert (
                turns_per_agent[agent_id] >= 8
            )  # Allow some variance due to threading
