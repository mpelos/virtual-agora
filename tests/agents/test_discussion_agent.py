...
from virtual_agora.agents.discussion_agent import (
    DiscussionAgent, AgentState, VoteType
)
...
class TestDiscussionAgent:
    ...
    def setup_method(self):
        ...
        self.mock_llm.with_fallbacks.return_value = self.mock_llm  # For error handling chain
...
class TestDiscussionAgentTopicProposals:
    ...
    def setup_method(self):
        ...
        self.agent = DiscussionAgent("test-agent", self.mock_llm, enable_error_handling=False)
...
class TestDiscussionAgentVoting:
    ...
    def test_vote_on_topic_conclusion_yes(self):
        ...
        mock_response = Mock()
        mock_response.content = "Yes, I think we have covered the main points adequately."
        self.mock_llm.invoke.return_value = mock_response
    ...
    def test_vote_on_conclusion_with_context(self):
        ...
        vote_result, reasoning = self.agent.vote_on_topic_conclusion("Test Topic", context)
...
class TestDiscussionAgentDiscussion:
    ...
    def setup_method(self):
        ...
        self.agent = DiscussionAgent("test-agent", self.mock_llm, max_context_messages=3, enable_error_handling=False)
...
class TestDiscussionAgentMinorityConsideration:
    ...
    def test_provide_minority_consideration(self):
        ...
        mock_response = Mock()
        mock_response.content = "I believe there's an important aspect we haven't fully explored."
        self.mock_llm.invoke.return_value = mock_response
        
        consideration = self.agent.provide_minority_consideration("Concluded Topic")
        
        assert consideration == "I believe there's an important aspect we haven't fully explored."
        self.mock_llm.invoke.assert_called_once()
    
    def test_provide_minority_consideration_with_context(self):
        """Test minority consideration with discussion context.""" 
        mock_response = Mock()
        mock_response.content = "Based on our discussion, I still have concerns about..."
        self.mock_llm.invoke.return_value = mock_response
        ...
        consideration = self.agent.provide_minority_consideration("Concluded Topic", context)
        
        assert consideration == "Based on our discussion, I still have concerns about..."
...
class TestDiscussionAgentPromptBuilding:
    ...
    def test_build_discussion_prompt_with_context(self):
        ...
        assert len(prompt.split("agent1:")[1].split("\n")[0]) <= 210  # 200 chars + "..."
...
class TestDiscussionAgentIntegration:
    ...
    def setup_method(self):
        ...
        self.agent = DiscussionAgent("integration-agent", self.mock_llm, enable_error_handling=False)
    ...
    def test_full_discussion_workflow(self, sample_context_messages):
        ...
        responses = [
            Mock(content="1. Ethics in AI\n2. Safety measures\n3. Regulatory frameworks"),  # Topic proposal
            Mock(content="I prefer topics 1, 3, 2 because ethics is fundamental"),  # Agenda vote
            Mock(content="Ethics in AI requires careful consideration of bias and fairness"),  # Discussion
            Mock(content="Yes, we have covered the main ethical concerns adequately"),  # Conclusion vote
            Mock(content="I wanted to add that transparency is also crucial for AI ethics")  # Minority consideration
        ]
        ...
        discussion_response = self.agent.generate_discussion_response("Ethics in AI", sample_context_messages)
        assert "careful consideration" in discussion_response
        
        # 4. Vote on conclusion
        vote_result, reasoning = self.agent.vote_on_topic_conclusion("Ethics in AI", sample_context_messages)
        assert vote_result is True
        assert "adequately" in reasoning
        assert len(self.agent.vote_history) == 2
        
        # 5. Provide minority consideration (if needed)
        consideration = self.agent.provide_minority_consideration("Ethics in AI", sample_context_messages)
