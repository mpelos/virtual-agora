...
class TestStory37MinorityConsiderations(TestModeratorNodes):
    ...
    @pytest.mark.asyncio
    async def test_identify_minority_voters_success(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_identify_minority_voters_no_active_vote(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_identify_minority_voters_no_votes(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_identify_minority_voters_unanimous(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_collect_minority_considerations_success(self, moderator_nodes, sample_state):
        ...
        result = await moderator_nodes.collect_minority_considerations_node(sample_state)
    ...
    @pytest.mark.asyncio
    async def test_collect_minority_considerations_no_minority(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_collect_minority_considerations_no_active_vote(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_collect_minority_considerations_method_failure(self, moderator_nodes, sample_state):
        ...
        assert "active_vote" in result
        assert "[Error collecting from agent2]" in result["active_vote"]["minority_considerations"]
    
    @pytest.mark.asyncio
    async def test_incorporate_minority_views_success(self, moderator_nodes, sample_state):
        ...
        assert result["topic_summaries"]["Test Topic"] == "Updated summary with minority views"
    ...
    @pytest.mark.asyncio
    async def test_incorporate_minority_views_no_considerations(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_incorporate_minority_views_method_failure(self, moderator_nodes, sample_state):
        ...
        moderator_nodes.moderator.incorporate_minority_views = AsyncMock(return_value=None)
...
class TestStory38ReportWriterMode(TestModeratorNodes):
    ...
    @pytest.mark.asyncio
    async def test_initialize_report_structure_success(self, moderator_nodes, sample_state):
        ...
        assert result["report_structure"] == ["Executive Summary", "Detailed Analysis", "Conclusions"]
        assert "report_sections" in result
        assert result["report_sections"] == {}
        assert result["report_generation_status"] == "structuring"
    
    @pytest.mark.asyncio
    async def test_initialize_report_structure_no_summaries(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_initialize_report_structure_method_failure(self, moderator_nodes, sample_state):
    ...
    async def test_generate_report_section_success(self, moderator_nodes, sample_state):
        ...
        assert "report_sections" in result
        assert result["report_sections"]["Executive Summary"] == "Generated executive summary content"
        assert result["report_generation_status"] == "writing"
    
    @pytest.mark.asyncio
    async def test_generate_report_section_all_completed(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_generate_report_section_no_structure(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_generate_report_section_method_failure(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_finalize_report_missing_sections(self, moderator_nodes, sample_state):
...
class TestStory39AgendaModification(TestModeratorNodes):
    ...
    @pytest.mark.asyncio
    async def test_request_agenda_modifications_success(self, moderator_nodes, sample_state):
        ...
        assert all("Suggested modification" in mod for mod in result["pending_agenda_modifications"])
    
    @pytest.mark.asyncio
    async def test_request_agenda_modifications_no_topics(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_request_agenda_modifications_method_failure(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_synthesize_agenda_changes_success(self, moderator_nodes, sample_state):
        """Test successful agenda change synthesis."""
        sample_state["pending_agenda_modifications"] = [
            "Add new topic X", "Remove topic 2"
        ]
    ...
    @pytest.mark.asyncio
    async def test_synthesize_agenda_changes_no_modifications(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_synthesize_agenda_changes_method_failure(self, moderator_nodes, sample_state):
        """Test handling of method failure during synthesis."""
        sample_state["pending_agenda_modifications"] = ["Add topic X"]
        
        # Mock method to return None (failure)
        moderator_nodes.moderator.synthesize_agenda_modifications = AsyncMock(return_value=None)
    ...
    @pytest.mark.asyncio
    async def test_facilitate_agenda_revote_success(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_facilitate_agenda_revote_no_topics(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_facilitate_agenda_revote_no_votes(self, moderator_nodes, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_facilitate_agenda_revote_synthesis_failure(self, moderator_nodes, sample_state):
...
class TestDirectNodeFunctions(TestModeratorNodes):
    ...
    @pytest.mark.asyncio
    async def test_direct_collect_minority_considerations(self, mock_moderator, sample_state):
        """Test direct collect_minority_considerations function."""
        sample_state["active_vote"]["minority_voters"] = ["agent2"]
        mock_moderator.collect_minority_consideration = AsyncMock(return_value="consideration")
    ...
    @pytest.mark.asyncio
    async def test_direct_initialize_report_structure(self, mock_moderator, sample_state):
    ...
    @pytest.mark.asyncio
    async def test_direct_request_agenda_modifications(self, mock_moderator, sample_state):
        """Test direct request_agenda_modifications function."""
        mock_moderator.request_agenda_modification = AsyncMock(return_value="modification")
    ...
    async def test_error_handling_consistency(self, moderator_nodes, sample_state):
        ...
        for node_function in node_functions:
            ...
            try:
                ...
                assert "last_error" in result or "warnings" in result or "report_generation_status" in result
