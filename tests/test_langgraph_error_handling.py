...
from virtual_agora.utils.error_handler import error_handler, ErrorContext, RecoveryStrategy
...
class TestLangGraphErrorHandler:
    ...
    def test_create_retry_policy(self):
        ...
        policy = handler.create_retry_policy(
            max_attempts=3,
    ...
    def test_node_error_summary(self):
        ...
        for i in range(3):
            error_handler.capture_langgraph_context(
                ProviderError(f"Error {i}"),
                graph_node="problem_node",
                graph_state={}
            )
