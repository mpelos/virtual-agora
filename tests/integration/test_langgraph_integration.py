...
class TestLangGraphIntegration:
    ...
    def test_streaming_in_graph(self, mock_create_provider):
        ...
        def streaming_node(state):
            ...
            return {"messages": [AIMessage(content="".join(chunks), name="stream-agent")]}
    ...
    def test_multi_agent_discussion_flow(self, mock_create_provider):
        ...
        for i, msg in enumerate(messages):
            if hasattr(msg, "name"):
                ...
                print(f"Message {i}: {type(msg)}, content={getattr(msg, 'content', 'N/A')[:50]}")
        ...
        assert any(name in speaker_contents for name in ["moderator", "gpt-4o_1", "gpt-4o_2"])
