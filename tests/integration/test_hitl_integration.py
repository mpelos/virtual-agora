...
class TestHITLIntegration:
    ...
    def test_hitl_gate_handling(self, app_v13):
        ...
        with patch.object(
            app_v13.hitl_manager, "process_interaction", return_value=mock_response
        ):
    ...
    def test_periodic_stop_force_end(self, app_v13):
        ...
        mock_response = HITLResponse(
            approved=True, action="force_topic_end", reason="User wants to end topic"
        )
    ...
    def test_agenda_approval_flow(self, app_v13):
        ...
        mock_response = HITLResponse(
            approved=True, action="edited", modified_data=edited_agenda
        )
    ...
    def test_session_continuation_override(self, app_v13):
        ...
        # Mock user override
        mock_response = HITLResponse(approved=True, action="continue_session")
    ...
    def test_interrupt_handling(self, app_v13):
        ...
        app_v13._handle_interrupt({"action": "end_topic", "reason": "User interrupt"})

        app_v13.state_manager.update_state.assert_called_with(
            {"user_forced_conclusion": True, "force_reason": "User interrupt"}
        )

        # Test skip to report interrupt
        app_v13._handle_interrupt(
            {"action": "skip_to_report", "reason": "User wants report"}
        )

        app_v13.state_manager.update_state.assert_called_with(
            {"topic_queue": [], "skip_to_report": True}
        )
    ...
    def test_checkpoint_notification_display(self, app_v13):
        ...
        with patch.object(
            app_v13.session_controller, "check_periodic_control", return_value=True
        ):
            with patch.object(
                app_v13.session_controller, "display_checkpoint_notification"
            ) as mock_display:
                # Create HITL state
                state["hitl_state"] = {
                    "awaiting_approval": True,
    ...
    def test_agent_display_integration(self, app_v13):
        ...
        app_v13.show_agent_invocation(
            agent_type="summarizer", task="Summarize discussion", context={"round": 5}
        )

        app_v13.dashboard.agent_display.show_agent_invocation.assert_called_once_with(
            "summarizer", "Summarize discussion", {"round": 5}, status="active"
        )

        # Test agent result display
        app_v13.show_agent_result(
            agent_type="summarizer", result="Summary complete", execution_time=1.5
        )

        app_v13.dashboard.agent_display.show_agent_result.assert_called_once_with(
            "summarizer", "Summary complete", 1.5
        )
    ...
    def test_run_session_with_hitl(
        self, mock_live, mock_get_topic, mock_spinner, app_v13
    ):
        ...
        # Mock HITL response
        mock_response = HITLResponse(
            approved=True, action="approved", modified_data=["Topic 1", "Topic 2"]
        )
    ...
    def test_checkpoint_creation_during_phases(self, app_v13):
        ...
        app_v13._handle_interrupt({"action": "pause", "checkpoint": checkpoint_data})
