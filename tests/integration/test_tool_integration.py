...
class TestAgentToolExecution:
    ...
    def test_agent_generates_tool_call(self):
        ...
        mock_ai_message = AIMessage(
            content="I'll propose some topics.",
        ...
        # Create state
        state = {
            "messages": [HumanMessage(content="Please propose some topics")]
        }
    ...
    def test_agent_executes_tool_calls(self):
        ...
        tool_call = {
            ...
            "name": "propose_topics",
            "args": {
                "topics": ["AI Safety", "Quantum Computing", "Biotechnology"]
            }
        }
        
        ai_msg = AIMessage(
            content="Let me propose topics.",
            tool_calls=[tool_call]
        )
        
        state = {
            "messages": [
                HumanMessage(content="Propose topics"),
                ai_msg
            ]
        }
    ...
    async def test_agent_async_tool_execution(self):
        ...
        mock_ai_message = AIMessage(
            content="I'll summarize the discussion.",
            tool_calls=[tool_call]
        )
        self.mock_llm.ainvoke = AsyncMock(return_value=mock_ai_message)
        
        state = {
            "messages": [HumanMessage(content="Please summarize")]
        }
    ...
    def test_agent_streaming_with_tools(self):
        ...
        chunks = [
            ...
            Mock(content="on the topic.", tool_calls=[{
                "id": "call_stream",
                "name": "vote",
        ...
        state = {
            "messages": [HumanMessage(content="Vote on AI Ethics")]
        }
...
class TestToolErrorHandling:
    ...
    def test_tool_validation_error(self):
        ...
        invalid_args = {
            "topics": ["Only one topic"],  # Needs 3-5
            "rationale": "Test"
        }
    ...
    def test_tool_error_recovery_strategy(self):
        ...
        strategy = ToolErrorRecoveryStrategy(
            max_retries=3,
            exponential_backoff=True
        )
        
        # Test retryable error
        retryable_error = ToolRetryableError(
            "Network error",
            tool_name="test_tool"
        )
