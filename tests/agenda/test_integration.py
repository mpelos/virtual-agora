...
class TestAgendaManagementIntegration:
    ...
    async def test_full_agenda_initialization_workflow(self):
        ...
        with patch(
            "virtual_agora.agenda.manager.ThreadPoolExecutor"
            ...
            for agent_id, agent in self.agents.items():
                ...
                future.result.return_value = [
                    Proposal(
                        agent_id=agent_id,
                        topic=f"{agent_id}_TopicA",
                        status=ProposalStatus.COLLECTED,
                    ),
                    Proposal(
                        agent_id=agent_id,
                        topic=f"{agent_id}_TopicB",
                        status=ProposalStatus.COLLECTED,
                    ),
                ]
            ...
            with patch(
                "virtual_agora.agenda.manager.as_completed",
                return_value=proposal_futures[::-1],
            ):
                # Mock voting collection futures
                with patch(
                    "virtual_agora.agenda.voting.ThreadPoolExecutor"
                ) as mock_vote_executor_class:
                    mock_vote_executor = Mock()
                    mock_vote_executor_class.return_value.__enter__.return_value = (
                        mock_vote_executor
                    )
                    ...
                    mock_vote_executor.submit.side_effect = (
                        lambda *args: vote_futures.pop(0)
                    )

                    # Mock as_completed for votes
                    with patch(
                        "virtual_agora.agenda.voting.as_completed",
                        return_value=vote_futures[::-1],
                    ):
                        # Execute the initialization
                        result_state = await self.manager.initialize_agenda(
                            self.agents, "Main Discussion Topic", proposals_per_agent=3
                        )
    ...
    async def test_agenda_initialization_with_empty_proposals(self):
        ...
        with patch(
            ...
            with patch(
                "virtual_agora.agenda.manager.as_completed",
                return_value=empty_futures[::-1],
            ):
                result_state = await self.manager.initialize_agenda(
                    self.agents, "Main Discussion Topic"
                )
        ...
        assert all(
            item.proposed_by == ["system"] for item in result_state.current_agenda
        )
    ...
    async def test_agenda_modification_workflow(self):
        ...
        self.moderator.request_agenda_modification.side_effect = (
            mock_request_modification
        )
        ...
        with patch(
            ...
            with patch(
                ...
                result_state = await self.manager.modify_agenda(
                    self.agents, remaining_topics
                )
    ...
    def test_topic_transition_workflow(self):
        ...
        topic_a = next(
            item
            for item in self.manager.state.current_agenda
    ...
    async def test_analytics_throughout_workflow(self):
        ...
        with patch.object(
            self.manager.proposal_collector, "collect_proposals"
        ) as mock_collect_proposals:
            with patch.object(
                self.manager.voting_orchestrator, "collect_votes"
            ) as mock_collect_votes:
                with patch.object(
                    self.manager.synthesizer, "synthesize_agenda"
                ) as mock_synthesize:
        ...
        transition = self.manager.transition_to_topic(
            "Test Topic", list(self.agents.keys())
        )
