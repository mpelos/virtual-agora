...
class TestVoteParser:
    ...
    def test_parse_empty_vote(self):
        ...
        preferences = self.parser.parse_vote("This doesn't mention any topics", self.topics)
        assert preferences == []
...
class TestVotingOrchestrator:
    ...
    def test_create_vote_prompt_modification(self):
        ...
        prompt = self.orchestrator._create_vote_prompt(topics, VoteType.AGENDA_MODIFICATION)
    ...
    async def test_collect_votes_success(self):
        ...
        for i, agent in enumerate(self.agents.values()):
            agent.vote_on_agenda.return_value = f"1. Topic {chr(65+i)}\n2. Topic {chr(66+i)}"
        ...
        with patch.object(self.orchestrator, '_collect_single_agent_vote', side_effect=mock_collect_vote):
