...
class TestModelInfo:
    ...
    def test_model_info_defaults(self):
        ...
        model = ModelInfo(
            name="test", display_name="Test", context_window=128000, output_tokens=1000
        )
        assert model.supports_streaming is True
...
class TestProviderInfo:
    ...
    def test_add_model(self):
        ...
        provider = ProviderInfo(
            provider_type=ProviderType.GOOGLE, display_name="Google"
        )
...
class TestProviderRegistry:
    ...
    def test_get_all_models(self):
        ...
        assert any(
            "Gemini 2.0 Flash (gemini-2.0-flash)" in model for model in google_models
        )
...
class TestNewRegistryFeatures:
    ...
    def test_get_model_capabilities(self):
        ...
        capabilities = reg.get_model_capabilities(ProviderType.GOOGLE, "non-existent")
    ...
    def test_supports_feature(self):
        ...
        # Test vision support
        assert (
            reg.supports_feature(ProviderType.GOOGLE, "gemini-2.0-flash", "vision")
            is True
        )

        # Test feature not supported
        assert reg.supports_feature(ProviderType.OPENAI, "o1-preview", "tools") is False

        # Test non-existent model
        assert (
            reg.supports_feature(ProviderType.GOOGLE, "non-existent", "vision") is False
        )
    ...
    def test_get_latest_model(self):
        ...
        assert (
            reg.get_latest_model(ProviderType.ANTHROPIC) == "claude-3-5-sonnet-20241022"
        )
