...
class TestProviderConfig:
    ...
    def test_temperature_validation(self):
        ...
        # Valid temperatures
        config = ProviderConfig(provider=ProviderType.GOOGLE, model="test", temperature=0.0)
        assert config.temperature == 0.0
        
        config = ProviderConfig(provider=ProviderType.GOOGLE, model="test", temperature=2.0)
    ...
    def test_max_tokens_validation(self):
        ...
        # Valid max_tokens
        config = ProviderConfig(provider=ProviderType.GOOGLE, model="test", max_tokens=1)
...
class TestCreateProviderConfig:
    ...
    def test_ignore_invalid_kwargs(self):
        ...
        config = create_provider_config(
            provider="openai",
            model="gpt-4",
            top_k=40  # Google/Anthropic specific
        )
        assert isinstance(config, OpenAIProviderConfig)
