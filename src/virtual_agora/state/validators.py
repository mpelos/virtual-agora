...
class StateValidator:
    ...
    def validate_phase_transition(
        ...
        if new_phase == 4:
            ...
            if state["topic_queue"] or state["active_topic"]:
                raise ValidationError(
                    "Cannot enter Summary phase with pending topics"
                )
    ...
    def validate_speaker(
        ...
        if state["current_speaker_id"] != speaker_id:
            ...
            raise ValidationError(
                f"It's not {speaker_id}'s turn to speak. "
    ...
    def validate_vote(
        ...
        existing_votes = [
            ...
            if v["voter_id"] == voter_id and 
            v.get("metadata", {}).get("vote_round_id") == vote_id
        ]
        if existing_votes:
            raise ValidationError(
                f"Agent {voter_id} has already voted in this round"
            )
    ...
    def validate_topic_transition(
        ...
        if new_topic and new_topic not in state["topic_queue"]:
            if new_topic not in state["completed_topics"]:
                raise ValidationError(
                    f"Topic '{new_topic}' is not in the queue"
                )
            else:
                raise ValidationError(
                    f"Topic '{new_topic}' has already been discussed"
                )
    ...
    def validate_message_format(self, message: Message) -> None:
        ...
        """
        required_fields = ["id", "speaker_id", "speaker_role", "content", 
                          "timestamp", "phase"]
        ...
        if message["speaker_role"] not in ["moderator", "participant"]:
            raise ValidationError(
                f"Invalid speaker role: {message['speaker_role']}"
            )
    ...
    def validate_agent_info(self, agent: AgentInfo) -> None:
        ...
        """
        required_fields = ["id", "model", "provider", "role", "message_count", 
                          "created_at"]
    ...
    def validate_speaking_order(
        ...
        if state["current_phase"] in [1, 2, 3]:
            participants = {
                aid for aid, info in state["agents"].items()
    ...
    def validate_state_consistency(self, state: VirtualAgoraState) -> List[str]:
        ...
        for topic, info in state["topics_info"].items():
            if info["status"] == "active" and topic != state["active_topic"]:
                warnings.append(
                    f"Topic '{topic}' marked as active but not current"
                )
