...
class StateRecoveryManager:
    ...
    def rollback_to_checkpoint(
        ...
        logger.info(
            f"Rolled back to checkpoint {checkpoint_id} " f"from {checkpoint.timestamp}"
        )
    ...
    def find_last_valid_state(self) -> Optional[StateCheckpoint]:
        ...
        if self.checkpoint_dir and self.checkpoint_dir.exists():
            checkpoint_files = sorted(
                self.checkpoint_dir.glob("checkpoint_*.pkl"), reverse=True
            )

            for checkpoint_file in checkpoint_files:
                try:
                    checkpoint = self._load_checkpoint_from_disk(checkpoint_file.stem)
                    if checkpoint:
                        is_valid, _ = self.validate_state(checkpoint.state_snapshot)
                        if is_valid:
                            return checkpoint
    ...
    def emergency_recovery(
        ...
        if hasattr(state_manager, "_state") and state_manager._state:
            ...
            if not is_valid:
                repaired = self.repair_corrupted_state(state_manager._state, warnings)
                if repaired:
        ...
        if last_valid:
            try:
                self.rollback_to_checkpoint(last_valid.checkpoint_id, state_manager)
                logger.info(
