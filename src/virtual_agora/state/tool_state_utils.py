...
def update_tool_metrics(
    current_metrics: Optional[ToolExecutionMetrics],
    new_tool_calls: List[ToolCallInfo]
) -> ToolExecutionMetrics:
    ...
    for call in new_tool_calls:
        ...
        metrics["calls_by_agent"][agent_id] = metrics["calls_by_agent"].get(agent_id, 0) + 1
...
def get_tool_usage_summary(state: Union[VirtualAgoraState, ToolEnabledState]) -> str:
    ...
    for tool, count in sorted(metrics["calls_by_tool"].items(), key=lambda x: x[1], reverse=True):
        summary_parts.append(f"  - {tool}: {count} calls")
    
    if metrics["calls_by_agent"]:
        summary_parts.extend(["", "By Agent:"])
        for agent, count in sorted(metrics["calls_by_agent"].items(), key=lambda x: x[1], reverse=True):
