...
class StateManager:
    ...
    def initialize_state(self, session_id: Optional[str] = None) -> VirtualAgoraState:
        ...
        if session_id is None:
            session_id = (
                datetime.now().strftime("%Y%m%d_%H%M%S_") + str(uuid.uuid4())[:8]
            )
    ...
    def transition_phase(
        self, new_phase: int, reason: str = "Normal progression"
    ) -> None:
    ...
    def add_message(
        self, speaker_id: str, content: str, metadata: Optional[Dict[str, Any]] = None
    ) -> Message:
    ...
    def start_vote(
        self, vote_type: str, options: List[str], required_votes: Optional[int] = None
    ) -> VoteRound:
        ...
        if required_votes is None:
            required_votes = len(
                [a for a in self.state["agents"].values() if a["role"] == "participant"]
            )
    ...
    def _complete_vote(self) -> None:
        ...
        participation = (
            vote_round["received_votes"] / total_agents if total_agents > 0 else 0
        )
