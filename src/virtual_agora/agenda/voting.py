...
from virtual_agora.agenda.models import (
    Vote, VoteType, VoteCollection, VoteStatus
)
...
class VoteParser:
    ...
    def parse_vote(self, vote_content: str, available_topics: List[str]) -> List[str]:
        ...
        mentioned_topics = self._extract_mentioned_topics(vote_content, available_topics)
        if mentioned_topics:
    ...
    def _extract_ranked_topics(self, vote_content: str, available_topics: List[str]) -> List[str]:
        ...
        for pattern in self.ranking_patterns:
            ...
            for match in matches:
                ...
                # Find matching topic
                matched_topic = self._find_best_topic_match(topic_text, available_topics)
                if matched_topic:
    ...
    def _extract_mentioned_topics(self, vote_content: str, available_topics: List[str]) -> List[str]:
    ...
    def _score_topics_by_context(self, vote_content: str, available_topics: List[str]) -> List[str]:
        ...
        # Positive indicators
        positive_words = ['prefer', 'like', 'important', 'priority', 'first', 'main', 'key']
        # Negative indicators
        ...
        for topic in available_topics:
            ...
            if topic_lower in vote_lower:
                ...
                context_after = vote_lower[topic_start + len(topic_lower):topic_start + len(topic_lower) + 50]
    ...
    def _find_best_topic_match(self, text: str, available_topics: List[str]) -> Optional[str]:
...
class VotingOrchestrator:
    ...
    async def collect_votes(
        ...
        """
        logger.info(f"Starting vote collection for {len(agents)} agents on {len(topic_options)} topics")
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=len(agents)) as executor:
                ...
                try:
                    ...
                    logger.warning(f"Vote collection timeout after {self.timeout_seconds}s")
            ...
            collection.votes = self._validate_and_parse_votes(collection.votes, topic_options)
    ...
    def _validate_and_parse_votes(
        ...
        for vote in votes:
            try:
                ...
                preferences = self.vote_parser.parse_vote(vote.vote_content, topic_options)
    ...
    def _calculate_confidence_score(
        ...
        if any(char.isdigit() for char in vote_content):
            ...
            if '\n' in vote_content and len([line for line in vote_content.split('\n') if line.strip().startswith(tuple('123456789'))]) >= 2:
                score += 0.1
    ...
    def analyze_vote_distribution(self, votes: List[Vote]) -> Dict[str, Any]:
        ...
        valid_votes = [v for v in votes if v.status == VoteStatus.SUBMITTED and v.parsed_preferences]
        
        if not valid_votes:
            return analysis
        
        # Calculate average confidence
        confidences = [v.confidence_score for v in valid_votes if v.confidence_score is not None]
        if confidences:
        ...
        for vote in valid_votes:
            for i, topic in enumerate(vote.parsed_preferences):
                ...
                position_analysis[position][topic] = position_analysis[position].get(topic, 0) + 1
