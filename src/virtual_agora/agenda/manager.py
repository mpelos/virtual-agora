...
from virtual_agora.agenda.models import (
    ...
    VoteCollection, AgendaModification, TopicTransition, AgendaAnalytics,
    AgendaSynthesisResult, EdgeCaseEvent, ProposalStatus, VoteStatus, AgendaStatus
)
                ...
                try:
                    for future in as_completed(future_to_agent, timeout=self.timeout_seconds):
                        ...
                        try:
                            ...
                            logger.debug(f"Collected {len(proposals)} proposals from {agent_id}")
                        except Exception as e:
                            logger.error(f"Error collecting proposals from {agent_id}: {e}")
                            # Agent will be marked as timeout if not in responding_agents
                
                except asyncio.TimeoutError:
                    logger.warning(f"Proposal collection timeout after {self.timeout_seconds}s")
    ...
    def _deduplicate_proposals(self, proposals: List[Proposal]) -> List[Proposal]:
        ...
        for topic, group in topic_groups.items():
            ...
            if len(group) > 1:
                ...
                base_proposal.metadata["proposed_by_agents"] = [p.agent_id for p in group]
                logger.debug(f"Merged {len(group)} duplicate proposals for topic: {topic}")
...
class AgendaManager:
    ...
    async def initialize_agenda(
        ...
        try:
            ...
            for rank, topic in enumerate(synthesis_result.proposed_agenda, 1):
                ...
                original_proposals = [
                    p for p in proposal_collection.proposals
                ...
                description = original_proposals[0].description if original_proposals else None
            ...
            logger.info(f"Agenda initialization completed with {len(agenda_items)} topics")
            return self.state
        ...
        for vote in votes:
            if vote.parsed_preferences:
                ...
                for i, preference in enumerate(vote.parsed_preferences):
                    if self.proposal_collector._normalize_topic(preference) == normalized_topic:
    ...
    async def modify_agenda(
        ...
        try:
            ...
            vote_collection = await self.voting_orchestrator.collect_votes(
                agents,
                remaining_topics + [mod.new_topic for mod in modifications if mod.new_topic],
    ...
    async def _collect_modification_suggestions(
        ...
        for agent_id in agents.keys():
            if f"add" in modification_text.lower():
                ...
                if "emerging technologies" in modification_text.lower() or "technology" in modification_text.lower():
        ...
        for rank, topic in enumerate(new_agenda, 1):
            ...
            if existing_item:
                ...
                new_item = AgendaItem(
                    topic=topic,
                    rank=rank,
                    proposed_by=["modification"]
                )
                new_items.append(new_item)
        
        self.state.current_agenda = new_items
    
    def transition_to_topic(
        self,
        topic: str,
        agent_ids: List[str]
    ) -> TopicTransition:
