...
from virtual_agora.agenda.models import (
    Proposal, Vote, AgendaItem, AgendaSynthesisResult, AgendaModification
)
...
class AgendaSynthesizer:
    ...
    def synthesize_agenda(
        ...
        """
        logger.info(f"Starting agenda synthesis with {len(proposals)} proposals and {len(votes)} votes")
        ...
        while synthesis_attempts < self.max_retries:
            ...
            try:
                ...
                vote_strings = [f"{vote.agent_id}: {vote.vote_content}" for vote in votes]
                ...
                if parsed_agenda:
                    ...
                    logger.info(f"Agenda synthesis completed successfully after {synthesis_attempts} attempts")
                    return result
                ...
                if synthesis_attempts < self.max_retries:
                    logger.info(f"Retrying synthesis (attempt {synthesis_attempts + 1}/{self.max_retries})")
        
        # If all attempts failed, use fallback synthesis
        logger.error(f"All synthesis attempts failed. Using fallback. Last error: {last_error}")
    ...
    def _calculate_topic_scores(
        ...
        if topic_scores:
            ...
            for score in topic_scores.values():
                score.normalized_score = score.raw_score / max_score if max_score > 0 else 0.0
    ...
    def _prepare_synthesis_context(
        ...
        scoring_data = []
        for topic, score in sorted(topic_scores.items(), key=lambda x: x[1].raw_score, reverse=True):
            scoring_data.append({
                "topic": score.topic,
                ...
                "average_position": sum(score.vote_positions) / len(score.vote_positions) if score.vote_positions else 0
            })
    ...
    def _parse_agenda_response(self, response: str) -> Optional[List[str]]:
        ...
        try:
            ...
            json_match = re.search(r'\{[^}]*"proposed_agenda"[^}]*\}', response, re.DOTALL)
            if json_match:
    ...
    def _calculate_confidence_score(
        ...
        for i, topic in enumerate(agenda):
            ...
            if normalized in topic_scores:
                ...
                position_confidence = max(0.1, 1.0 - (i * 0.1))  # Decreases with position
    ...
    def _generate_explanation(
        ...
        explanation_parts.append("Agenda synthesized based on agent proposals and voting preferences:")
    ...
    def _fallback_synthesis(
        ...
        return AgendaSynthesisResult(
            ...
            tie_breaks_applied=["Fallback synthesis - no moderator tie-breaking applied"],
    ...
    def synthesize_modified_agenda(
        ...
        """
        logger.info(f"Synthesizing modified agenda with {len(modifications)} modifications")
        ...
        for item in current_agenda:
            if item.topic not in [item.topic for item in current_agenda if item.status == "completed"]:
                proposal = Proposal(
