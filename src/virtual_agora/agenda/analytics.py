...
from virtual_agora.agenda.models import (
    Proposal, Vote, VoteCollection, ProposalCollection, AgendaState,
    AgendaAnalytics, TopicTransition, AgendaModification, EdgeCaseEvent
)
...
class AgendaAnalyticsCollector:
    ...
    def record_topic_transition(self, transition: TopicTransition) -> None:
        ...
        logger.debug(f"Recorded topic transition: {transition.from_topic} -> {transition.to_topic}")
    ...
    async def record_agenda_modification(
        ...
        self.timeline_events.append({
            ...
            "details": {
                ...
                "modification_types": [mod.modification_type for mod in modifications]
            }
        })
    ...
    def generate_analytics(
        ...
        analytics.total_proposals = sum(len(pc.proposals) for pc in self.proposal_collections)
        ...
        if analytics.total_proposals > 0:
            ...
            analytics.proposal_acceptance_rate = accepted_topics / analytics.total_proposals
        
        # Vote participation metrics
        if self.vote_collections:
            participation_rates = [vc.participation_rate for vc in self.vote_collections]
        ...
        completed_transitions = [t for t in self.topic_transitions if t.transition_type == "complete"]
        
        if completed_transitions:
            durations = [t.duration_seconds for t in completed_transitions if t.duration_seconds]
            if durations:
                analytics.average_topic_duration_minutes = statistics.mean(durations) / 60
        ...
        # Timeline events
        analytics.timeline_events = sorted(self.timeline_events, key=lambda x: x["timestamp"])
    ...
    def _analyze_voting_patterns(self) -> Dict[str, Any]:
        ...
        if valid_votes:
            ...
            if topic_positions:
                ...
                if controversies:
                    patterns["most_controversial_topic"] = min(controversies, key=controversies.get)
                    patterns["most_consensus_topic"] = min(consensuses, key=consensuses.get)
    ...
    def generate_participation_report(self) -> Dict[str, Any]:
        ...
        report["overview"] = {
            ...
            "overall_participation_rate": total_participations / total_opportunities if total_opportunities > 0 else 0
        }
        ...
        low_participation_agents = [
            agent_id for agent_id, rate in agent_participation.items() 
            if rate < 0.7
        ]
