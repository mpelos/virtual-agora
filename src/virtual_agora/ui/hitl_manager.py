...
class EnhancedHITLManager:
    ...
    def _display_context(self, context: Dict[str, Any]) -> None:
        ...
        if "active_topic" in context:
            self.console.print(
                f"[bold]Current Topic:[/bold] {context['active_topic']}"
            )
    ...
    def _handle_topic_continuation(
        self,
        interaction: HITLInteraction
    ) -> HITLResponse:
        """Handle topic continuation approval."""
        completed_topic = interaction.context.get(
            "completed_topics", ["Unknown"]
        )[-1]
    ...
    def _handle_session_continuation(
        ...
        if agent_vote.get("recommendation") == "end":
            status = create_status_panel(
                "Agents recommend ending the session",
                style="warning"
            )
        else:
            status = create_status_panel(
                "Agents recommend continuing the session",
                style="info"
            )
    ...
    def _handle_agent_poll_override(
        self,
        interaction: HITLInteraction
    ) -> HITLResponse:
    ...
    def _handle_theme_input(
        ...
        # Get theme from user
        theme = Prompt.ask(
            interaction.prompt_message or "Enter discussion theme"
        )
        
        return HITLResponse(
            approved=True,
            action="theme_provided",
            modified_data=theme
        )
