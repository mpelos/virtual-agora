...
class ResponseValidator:
    ...
    def _validate_discussion_response(
        ...
        if context and "topic" in context:
            ...
            response_words = set(
                word for word in response_lower.split() if len(word) > 3
            )

            # Check for some overlap
            if topic_words and not topic_words.intersection(response_words):
                result.warnings.append(
                    "Response may not be directly related to the topic"
                )
    ...
    def _validate_conclusion_vote(
        ...
        if not has_yes and not has_no:
            ...
            if not (
                ...
                result.warnings.append(
                    "Vote contains both 'Yes' and 'No', may be ambiguous"
                )
    ...
    def _clean_response(self, response: str, response_type: ResponseType) -> str:
        ...
        if (
            len(cleaned) > self.max_response_length
    ...
    def validate_with_retry(
        ...
        while not result.is_valid and retry_count < self.max_retries:
            logger.info(
                f"Retrying validation for {agent_id}, attempt {retry_count + 1}"
            )
...
class ValidationMiddleware:
    ...
    def validate_agent_response(
        self, agent, response_method: str, response_type: ResponseType, *args, **kwargs
        ...
        if self.log_validation_results:
            if result.is_valid:
                logger.debug(
                    f"Validation passed for {agent.agent_id} {response_type.value}"
                )
            else:
                logger.warning(
