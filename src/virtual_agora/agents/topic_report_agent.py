    ...
    def synthesize_topic(
        ...
        if final_considerations:
            considerations_text = "\n\nFinal Considerations:\n" + \
                "\n".join([f"- {consideration}" for consideration in final_considerations])
...
Write as an objective analyst, not a participant. Ensure the report is thorough and self-contained."""
        ...
        logger.info(f"Generated topic report for '{topic}' with {len(round_summaries)} rounds")
...
Write objectively without taking sides."""
        ...
        logger.info(f"Synthesized {len(dissenting_agents)} minority views for topic '{topic}'")
...
Create a unified summary of these {len(chunk)} rounds, preserving key points and insights."""
        ...
        logger.info(f"Map-reduce: Processing {len(chunk_summaries)} chunks for topic '{topic}'")
        
        # Recursively reduce if needed
        if len(chunk_summaries) > chunk_size:
            return self.generate_topic_summary_map_reduce(chunk_summaries, topic, chunk_size)
