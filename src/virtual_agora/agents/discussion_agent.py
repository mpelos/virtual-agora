...
class DiscussionAgent(LLMAgent):
    ...
    def _build_topic_proposal_prompt(
        ...
        if context_messages:
            prompt_parts.insert(1, "Consider the following context from our discussion:")
            for msg in context_messages[-3:]:  # Last 3 messages for context
                prompt_parts.insert(2, f"- {msg['speaker_id']}: {msg['content'][:100]}...")
    ...
    def vote_on_topic_conclusion(
        ...
        """
        if self.is_muted():
            logger.warning(f"Muted agent {self.agent_id} cannot vote on topic conclusion")
        ...
        try:
            ...
            return False, "I apologize, but I'm having difficulty processing the conclusion vote."
        ...
        if context_messages:
            ...
            if recent_messages:
                ...
                for msg in recent_messages:
                    prompt_parts.append(f"- {msg['speaker_id']}: {msg['content'][:150]}...")
    ...
    def _parse_conclusion_vote(self, response: str) -> Tuple[bool, str]:
        ...
        if response_lower.startswith('yes'):
            ...
            if 'yes' in first_sentence and 'no' not in first_sentence:
                ...
                vote_result = False
                logger.warning(f"Unclear vote response from {self.agent_id}: {response[:100]}")
        ...
        if reasoning.lower().startswith(('yes', 'no')):
            ...
            reasoning = re.sub(r'^(yes|no)[.,\s]*', '', reasoning, flags=re.IGNORECASE).strip()
    ...
    def generate_discussion_response(
        ...
        """
        if self.is_muted():
            logger.warning(f"Muted agent {self.agent_id} cannot participate in discussion")
            return ""
        ...
        try:
            ...
            logger.error(f"Error generating discussion response for agent {self.agent_id}: {e}")
    ...
    def provide_minority_consideration(
        ...
        """
        if self.is_muted():
            logger.warning(f"Muted agent {self.agent_id} cannot provide minority consideration")
            return ""
        ...
        try:
            ...
            logger.error(f"Error in minority consideration for agent {self.agent_id}: {e}")
    ...
    def _build_minority_consideration_prompt(
        ...
        if context_messages:
            ...
            relevant_messages = [msg for msg in context_messages if msg.get('topic') == topic]
