...
class EnvironmentConfig(BaseSettings):
    ...
    google_api_key: Optional[str] = Field(
        None, alias="GOOGLE_API_KEY", description="Google Gemini API key"
    )
    ...
    log_dir: str = Field(
        "logs", alias="VIRTUAL_AGORA_LOG_DIR", description="Directory for log files"
    )
    ...
    debug_mode: bool = Field(
        False, alias="VIRTUAL_AGORA_DEBUG", description="Enable debug mode"
    )
    ...
    def get_api_keys(self) -> Dict[str, Optional[APIKeyConfig]]:
        ...
        return {
            "Google": (
                APIKeyConfig(key=self.google_api_key, provider="Google")
                if self.google_api_key
                else None
            ),
            "OpenAI": (
                APIKeyConfig(key=self.openai_api_key, provider="OpenAI")
                if self.openai_api_key
                else None
            ),
            "Anthropic": (
                APIKeyConfig(key=self.anthropic_api_key, provider="Anthropic")
