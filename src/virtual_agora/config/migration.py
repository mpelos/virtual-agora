...
def migrate_config_v1_to_v3(old_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if 'summarizer' not in new_config:
        logger.info(f"Adding default summarizer configuration using provider: {moderator_provider}")
    ...
    if 'topic_report' not in new_config:
        ...
        logger.info(f"Adding default topic_report configuration using provider: {topic_provider}")
    ...
    if 'ecclesia_report' not in new_config:
        logger.info(f"Adding default ecclesia_report configuration using provider: {moderator_provider}")
...
def validate_migrated_config(config: Dict[str, Any]) -> bool:
    ...
    """
    required_fields = ['moderator', 'summarizer', 'topic_report', 'ecclesia_report', 'agents']
    ...
    for field in required_fields:
        ...
        if field != 'agents':
            ...
            if not isinstance(agent_config, dict):
                logger.error(f"Invalid config for {field}: expected dict, got {type(agent_config)}")
                return False
...
def migrate_config_with_validation(old_config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if version == 'unknown':
        raise ValueError(f"Unknown configuration version, cannot migrate. Config: {old_config}")
