...
class ProviderFactory:
    ...
    def create_provider(
        ...
        try:
            ...
            logger.debug(f"Successfully created provider using init_chat_model: {config.provider}:{config.model}")
        except (ImportError, ModuleNotFoundError) as e:
            # Missing dependencies - try legacy method
            logger.info(f"init_chat_model failed due to missing dependencies, falling back to legacy method: {e}")
            ...
            logger.warning(f"init_chat_model failed unexpectedly, falling back to legacy method: {e}")
            provider_instance = cls._create_provider_instance_legacy(config)
    ...
    @classmethod
    def _add_provider_specific_params(cls, config: ProviderConfig, init_kwargs: Dict[str, Any]) -> None:
        ...
        logger.debug(f"Added provider-specific parameters for {config.provider}: {list(init_kwargs.keys())}")
    ...
    @classmethod
    def _create_provider_instance_legacy(
        cls,
        config: ProviderConfig
    ) -> BaseChatModel:
    ...
    @classmethod
    def _create_grok_provider_legacy(
        cls,
