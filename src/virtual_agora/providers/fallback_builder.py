...
class FallbackChainBuilder:
    ...
    def _sort_providers_by_strategy(self) -> List[ProviderPriority]:
        ...
        if self.strategy == FallbackStrategy.SEQUENTIAL:
            ...
            return sorted(
                self.providers,
                key=lambda p: (p.cost_per_token is None, p.cost_per_token or float('inf'))
            )
            ...
            return sorted(
                self.providers,
                key=lambda p: (p.average_latency is None, p.average_latency or float('inf'))
            )
...
class FallbackMonitor:
    ...
    def __init__(
        self,
        providers: List[ProviderPriority],
