...
def create_error_tool_message(
    tool_call_id: str, tool_name: str, error: Exception, include_traceback: bool = False
) -> ToolMessage:
...
def handle_tool_error(
    ...
    retryable_errors = (NetworkTransientError, TimeoutError, ToolRetryableError)
...
def validate_tool_input(tool: BaseTool, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if isinstance(
                        e, (NetworkTransientError, TimeoutError, ToolRetryableError)
                    ):
