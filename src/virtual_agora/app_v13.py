...
class VirtualAgoraApplicationV13:
    ...
    def handle_hitl_gate(self, state: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (interaction.approval_type == HITLApprovalType.PERIODIC_STOP and
            self.session_controller.check_periodic_control(state.get("current_round", 0))):
    ...
    def _process_hitl_response(
        ...
        if approval_type == HITLApprovalType.PERIODIC_STOP:
            ...
            state_updates["user_approves_continuation"] = response.action == "continue_session"
        ...
        state_updates["hitl_state"]["approval_history"] = state.get("hitl_state", {}).get("approval_history", []) + [{
