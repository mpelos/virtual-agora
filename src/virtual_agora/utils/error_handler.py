...
class ErrorHandler:
    ...
    def should_circuit_break(
        self, error_type: Type[Exception], threshold: int = 5
    ) -> bool:
    ...
    def register_recovery_callback(
        self, error_type: Type[Exception], callback: Callable[[ErrorContext], None]
    ) -> None:
            ...
            for error_type in error_types:
                if isinstance(error, error_type):
                    ...
                    if self.should_circuit_break(type(error)):
                        logger.warning(
                            f"Circuit breaker open for {type(error).__name__}"
                        )
                        return False
                    return True
            return False
    ...
    def create_error_recovery_node(
        self, recovery_strategies: Optional[Dict[str, RecoveryStrategy]] = None
    ) -> Callable:
    ...
    def get_node_error_summary(self, node_name: str) -> Dict[str, Any]:
        ...
        """
        node_errors = [ctx for ctx in self.error_history if ctx.graph_node == node_name]
